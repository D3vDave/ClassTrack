...
if (isset($_POST['change_username'])) {
    ...
    if ($new_username === '') {
        ...
        $u_stmt->bind_param("i", $teacher_id);
        $u_stmt->execute();
        ...
        if (!$hashed_pw || !password_verify($password, $hashed_pw)) {
            ...
            $check->bind_param("si", $new_username, $teacher_id);
            $check->execute();
            $check->store_result();
            if ($check->num_rows > 0) {
                ...
                $upd->bind_param("si", $new_username, $teacher_id);
                if ($upd->execute()) {
...
if (isset($_POST['change_password'])) {
    ...
    if ($new_password === '' || $confirm_password === '' || $old_password === '') {
        ...
        $p_stmt->bind_param("i", $teacher_id);
        $p_stmt->execute();
        ...
        if (!$current_hashed || !password_verify($old_password, $current_hashed)) {
            ...
            $u_stmt->bind_param("si", $new_hashed, $teacher_id);
            if ($u_stmt->execute()) {
...
if (isset($_POST['confirm_delete_account'])) {
    ...
    if ($confirm_username !== $current_username) {
        ...
        $d_stmt->bind_param("i", $teacher_id);
        $d_stmt->execute();
        ...
        if (!$pw_hash_for_delete || !password_verify($confirm_password, $pw_hash_for_delete)) {
            ...
            try {
                $del_ann = $conn->prepare("DELETE FROM announcements WHERE teacher_id = ?");
                $del_ann->bind_param("i", $teacher_id);
                $del_ann->execute();
                $del_ann->close();

                $del_sched = $conn->prepare("DELETE FROM class_schedules WHERE teacher_id = ?");
                $del_sched->bind_param("i", $teacher_id);
                $del_sched->execute();
                $del_sched->close();

                $del_user = $conn->prepare("DELETE FROM users WHERE id = ?");
                $del_user->bind_param("i", $teacher_id);
                $del_user->execute();
                $del_user->close();
...
<head>
<meta charset="utf-8" />
<title>Teacher Settings | ClassTrack</title>
...
<div class="dashboard">
  ...
  <div class="profile-simple">
    ...
    <ul class="menu">
        ...
        <li class="myschedule.php"><a href="myschedule.php"><i class="fas fa-calendar-alt"></i> My Schedule</a></li>
  ...
  <div class="content">
    ...
    <div class="cards">
      ...
      <div class="card">
        ...
        <p>This will permanently remove your account, schedules, and announcements.</p>
        <div class="form-actions">
